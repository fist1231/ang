    ...
    */
    ...
    myTest2( myaccount ): Observable<any> {
        ...
        const obs$ = Observable.create( observer => {
            ...
            /* test data            
                        ...
                        observer.next(77);
            */
            const cnt$ = interval( 1 ).scan( x => x + 1 ).takeWhile( x => x <= 10000 ); // web3.eth.blockNumber );
            cnt$.subscribe(
                x => {
                    web3.eth.getBlock( x, true, ( error, block ) => {
                        if ( !error )
    ...
    fillBlocks( numberOfBlocks ): Observable<any> {
        ...
        const obs$ = Observable.create( observer => {
            ...
            const cnt$ = interval( 1 ).scan( x => x + 1 ).takeWhile( x => x <= numberOfBlocks );
            cnt$.subscribe(
                x => {
